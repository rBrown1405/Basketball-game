...
(function () {
  ...
  function usageW(ctx, c) {
    const T = ctx.O;
    let w = Math.pow(scoreSkill(c.r) / 70, Sim.K.usageExp);
    const goToMod = C.OFFENSES[T.strat.off].mods.goTo || 1;
    if (T.strat.goTo1 === c.id) w *= 1.1 * goToMod;
    else if (T.strat.goTo2 === c.id) w *= 1.08 * (goToMod > 1 ? 1 + (goToMod - 1) * 0.4 : 1);
    w *= 1 + c.hot * 0.08;
    w *= fatigueMult(c);
    // teammates get involved once someone has jacked up a lot of shots
    w /= 1 + Math.max(0, c.st.fga + c.st.fta * 0.44 - 15) * 0.085;
  ...
  function subs(ctx, idx, start) {
    ...
    const crunch = (isEnd && clockLeft <= 300 && Math.abs(lead) <= 10) || g.period > L.periods;
    const garbage = isEnd && g.period === L.periods && ((Math.abs(lead) >= 20 && clockLeft <= 420) || (Math.abs(lead) >= 27 && clockLeft <= 700));
    ...
    if (manual.length) {
      ...
      const pri = c => {
        ...
        if (garbage) v = -c.target * 0.35 + (c.target <= 0 ? 6 : 0) + (c.energy - 80) / 10;
          ...
          v = (c.target * 60 * frac - c.sec) / 60 + (c.energy - 80) / 6.5;
          if (c.target <= 0) v -= 40;
  ...
  function actionTime(ctx, info, mode, clockLeft, scLeft) {
    ...
    if (info.play === 'putback') dt = U.range(0.4, 1.5);
      ...
      const mean = (g.L.key === 'women' ? Sim.K.shotTimeW : Sim.K.shotTime) - paceAdj * 0.36;
      const target = U.clamp(U.gauss(mean, 4.1), 2.5, ctx.scLen - 0.3);
  ...
  function choosePlay(ctx, mode, opts) {
    ...
    const on = O.on;
    const bestPost = Math.max(...on.map(c => c.r.post));
    const avg3 = avgOn(O, 'three');
    w.post *= U.clamp((bestPost - 58) / 18, 0.25, 1.6);
    ...
    switch (play) {
      case 'pnr': {
        info.handler = gim && gim.handler ? on.find(c => c.id === gim.handler) || pickFrom(on, handlerW) : pickFrom(on, handlerW);
      ...
      case 'iso': {
        info.handler = gim && gim.shooter ? on.find(c => c.id === gim.shooter) : pickFrom(on, c => Math.pow(usageW(ctx, c), 1.25) * (c.posN <= 3 ? 1 : 0.55));
        info.setName = U.pick(SET_NAMES.iso);
        break;
      }
      case 'post': {
        info.poster = gim && gim.shooter ? on.find(c => c.id === gim.shooter) : pickFrom(on, c => Math.pow(c.r.post / 60, 2.5) * (c.posN >= 3 ? 1 : 0.3) * usageW(ctx, c));
      ...
      case 'offscreen': {
        info.handler = pickFrom(on, handlerW);
        info.shooter = gim && gim.shooter ? on.find(c => c.id === gim.shooter) : pickFrom(others([info.handler]), c => Math.pow(Math.max(c.r.three, c.r.mid) / 65, 5) * usageW(ctx, c) * (c.posN <= 3 ? 1 : 0.3));
      ...
      case 'cut': {
        ...
        info.cutter = pickFrom(others([info.handler]), c => Math.pow((c.r.layup + c.r.dunk + c.r.speed) / 3 / 65, 4) * usageW(ctx, c));
  ...
  function zoneTendency(c, zone) {
    const r = c.r;
    const f = {
      rim: Math.pow(Math.max(r.layup, r.dunk, r.close) / 70, 2),
      paint: Math.pow(Math.max(r.close, r.post, (r.mid + r.layup) / 2) / 70, 2),
      mid: Math.pow(r.mid / 70, 2.2),
      c3: Math.pow(U.clamp((r.three - 38) / 32, 0.03, 2.2), 1.6),
      ab3: Math.pow(U.clamp((r.three - 38) / 32, 0.03, 2.2), 1.6),
    };
    return f[zone];
  }

  function chooseZone(ctx, c, base) {
    const O = ctx.O, D = ctx.D, L = ctx.g.L;
    ...
    for (const z in base) {
      ...
      if (is3(z)) v *= (off.three || 1) * L.threeRate * Sim.K.threeFreq;
  ...
  function pickShooter(ctx, exclude, kind) {
    ...
    if (kind === 'spot') return U.pickW(cands, c => Math.pow(U.clamp((c.r.three - 35) / 35, 0.05, 2), 3) * Math.sqrt(usageW(ctx, c)));
    if (kind === 'finisher') return U.pickW(cands, c => Math.pow((c.r.layup + c.r.dunk + c.r.speed) / 3 / 65, 3) * (c.posN >= 2 ? 1 : 0.6));
    return U.pickW(cands, c => usageW(ctx, c));
  }

  function chooseKind(zone, hint, c, lob) {
    const r = c.r;
    if (zone === 'rim') {
      if (hint === 'putback') return U.chance(0.4) ? 'tip' : (r.dunk >= 65 && U.chance(0.35) ? 'dunk' : 'layup');
      const m = hint === 'roll' || hint === 'cut' || hint === 'transition' ? 1.35 : hint === 'offDribble' ? 0.6 : 1;
      if (U.chance(U.clamp((r.dunk - 46) / 52, 0, 0.9) * m)) return lob && U.chance(0.55) ? 'alley' : 'dunk';
    ...
    if (zone === 'mid') {
      if (hint === 'catch') return 'jumper';
      if (hint === 'post') return U.chance(0.55) ? 'fadeaway' : 'jumper';
      if (hint === 'iso') return U.pickKey({ pullup: 45, stepback: 25, fadeaway: 30 });
      return 'pullup';
    }
    if (hint === 'catch') return 'catch_shoot';
    if (hint === 'iso') return U.chance(0.42) ? 'stepback' : 'pullup';
    if (hint === 'offDribble' || hint === 'transition') return U.chance(0.18) ? 'stepback' : 'pullup';
    return 'catch_shoot';
  }
  ...
  function planShot(ctx, info, mode) {
    ...
    const gim = ctx.P.gim;
    const on = O.on;
    let branch = U.pickKey(BRANCHES[info.play] || { self: 1 });
    ...
    if (gim) {
      ...
      branch = 'gim';
      return { branch, shooter, assister, zone: z, kind: gim.kind || chooseKind(z, hint, shooter, false), cKey, passKind };
    }
    switch (info.play) {
      case 'pnr':
      ...
      case 'spot':
        ...
        else { shooter = pickShooter(ctx, [], 'usage'); assister = shooter !== info.handler && U.chance(0.6) ? info.handler : null; base = { rim: 58, paint: 26, mid: 16 }; hint = 'offDribble'; cKey = 'drive'; }
        break;
      case 'offscreen':
    ...
    const zone = mode === 'lastShot' && ctx.heave ? 'ab3' : chooseZone(ctx, shooter, base);
    let kind = chooseKind(zone, hint, shooter, lob);
  ...
  function makeProb(ctx, sh, zone, kind, contest, d, info) {
    ...
    if (O.idx === 0) x += 0.025;
    if (isClutch(g)) x += (r.clutch - 70) / 10 * 0.07;
  ...
  function shootingFoulProb(ctx, zone, sh, d, kind) {
    ...
    if (ctx.O.idx === 0) p *= 1.03; else p *= 0.97;
    if (ctx.transition && ctx.segN <= 1) p *= 1.15;
    return U.clamp(p, 0.003, 0.45);
  }

  function ftProb(ctx, c) {
    let p = Sim.K.ftA + 0.0066 * c.r.ft + ctx.g.L.ftBase;
    if (isClutch(ctx.g)) p += (c.r.clutch - 70) * 0.0012;
    p -= Math.max(0, 68 - c.energy) * 0.001;
    if (ctx.O.idx === 1) p -= 0.004;
    return U.clamp(p, 0.3, 0.96);
  }
  ...
  function rebound(ctx, shot, zone) {
    ...
    const reb = U.pickW(T.on, c => Math.pow(c.r[key] / 50, 1.15) * posF[c.posN] * (0.7 + c.r.hustle / 230) * (c.id === shot.shooter ? 0.8 : 1));
    reb.st[off ? 'orb' : 'drb']++;
  ...
  function turnover(ctx, info, t, forceKind) {
    ...
    const stolen = (kind === 'bad_pass' && U.chance(Sim.K.stlBad)) || (kind === 'lost_ball' && U.chance(Sim.K.stlLost));
    const bx = basketX(O.idx, g.period), dir = dirX(O.idx, g.period);
    ...
    let stealer = null;
    if (stolen) {
      stealer = U.pickW(D.on, c => Math.pow(c.r.steal / 55, 1.5) * (c.r.agility / 70));
      stealer.st.stl++;
    }
  ...
  function nonShootingFoul(ctx, info, t) {
    ...
    const fouler = U.pickW(D.on, c => (100 - c.r.helpD) * (0.6 + c.r.steal / 150) * (c.pf >= foulLimit(g.period, L) ? 0.4 : 1));
  ...
  function tick(g, dt) {
    for (const T of g.t) {
      ...
      for (const c of T.players) {
        if (c.on) {
          c.sec += dt;
          const drain = 0.03 * (1.36 - c.r.stamina / 140) * (own.fatigue || 1) * offF * presF * paceF;
  ...
  function injuryCheck(ctx, dt) {
    ...
    for (const T of g.t) for (const c of T.on) {
      if (c.inj) continue;
      const p = 0.0000105 * dt * (1.65 - c.r.durability / 100) * (c.energy < 50 ? 1.4 : 1);
      if (U.chance(p)) {
        c.inj = true;
        c.injNew = PBC.Player.genInjury();
